...
$sql = "SELECT * FROM configura";
$result = $conn->query($sql);
$row = mysqli_fetch_array($result);

$nomedosite = stripslashes(utf8_encode($row['nomedosite']));
$email_admin = stripslashes($row['email_admin']);
$email_copia = stripslashes($row['email_copia']);
$tel_admin = stripslashes(utf8_encode($row['tel_admin']));
$tel_admin1 = stripslashes(utf8_encode($row['tel_admin1']));
$tel_admin2 = stripslashes(utf8_encode($row['tel_admin2']));
$tel_admin3 = stripslashes(utf8_encode($row['tel_admin3']));


if($_GET['act'] && $_GET['act'] == "go" 
	&& isset($_POST['nome']) && $_POST['nome'] != "" && isset($_POST['email']) && $_POST['email'] != "" 
	&& isset($_POST['cidade']) && $_POST['cidade'] != "" && isset($_POST['estado']) && $_POST['estado'] != "" && $_POST['profissao'] != "") {

$token = "--";		
$parceiro = "WebJuris";	
$fromsite = "https://previus.webjuris.com.br";
...
$userip = trim($_POST['ip']);
if(isset($_POST['fromsite']) && $_POST['fromsite'] != '') {
  $fromsite = trim($_POST['fromsite']);
}
$profissao = trim($_POST['profissao']);
$hour = date("H");
$date = date('d/m/Y H:i:s',mktime($hour-2));	
...
//$assunto = "Solicitação de Teste - ".$programa;
...
$conn->query("INSERT INTO contr_cadastros (nome, email, telefone, cidade, estado, profissao, produto, vendedor, msg_id, novo, response, retorno, site, data, ip, hora) 
			VALUES(
					'".utf8_decode($nome)."',
					'".$email."',
					'".$telefone."',
...
Em breve um dos nossos colaboradores especializados poderá te ligar colocando a sua disposição o nosso <span color="#800000">atendimento e treinamento especializado gratuito</span>.<br /><br />
